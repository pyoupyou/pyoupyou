...
def activity_summary(request):
    ...
    closed_processes = process_filter.qs.filter(state__in=Process.CLOSED_STATE_VALUES).values("subsidiary__name").annotate(count=Count("candidate"))
    ...
    new_interviews = interview_filter.qs.order_by("process__subsidiary__name").values("process__subsidiary__name").annotate(count=Count("id"))
    print(new_interviews)
    # New GO interviews
    new_interviews_go_total = interview_filter.qs.filter(state=Interview.GO).count()
    new_interviews_go = interview_filter.qs.filter(state=Interview.GO).order_by("process__subsidiary__name").values("process__subsidiary__name").annotate(count=Count("id"))
